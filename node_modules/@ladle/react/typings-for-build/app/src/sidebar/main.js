...
const Main = ({ stories, story, updateStory, hotkeys, search, setSearch, }) => {
    ...
    const onKeyDown = (e) => {
        if (e.key === "ArrowDown" && treeRoot.current?.firstChild) {
            const firstLiElement = treeRoot.current.firstChild;
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { role: "separator", "aria-orientation": "vertical", ref: handleRef, className: cx("ladle-resize-handle", {
                ...
                } }), _jsxs("nav", { role: "navigation", className: "ladle-aside", style: { minWidth: `${width}px` }, children: [_jsx("input", { placeholder: "Search", "aria-label": "Search stories", value: search, ref: searchEl, onKeyDown: onKeyDown, onChange: (e) => setSearch(e.target.value) }), _jsx(TreeView, { searchRef: searchEl, stories: filteredStories, story: story, hotkeys: hotkeys, updateStory: updateStory, allExpanded: search !== "" || config.expandStoryTree, setTreeRootRef: (root) => (treeRoot.current = root) })] })] }));
};
export default Main;
