...
import { stories, storySource, StorySourceHeader, } from "virtual:generated-list";
...
export const CodeHighlight = ({ children, theme, language = "tsx", locStart, locEnd, className, }) => {
    ...
    if (match) {
        language = match[1];
        return (_jsx(Highlight, { code: children.trim(), language: language, theme: theme === "dark"
                ? config.addons.source.themeDark
                : config.addons.source.themeLight, children: ({ className, style, tokens, getTokenProps }) => (_jsx("div", { className: className, style: {
                    ...style,
                    textAlign: "left",
                    margin: "0.5em 0 1em 0",
                    padding: "1em",
                }, children: tokens.map((line, i) => (_jsx("div", { children: line.map((token, key) => (_jsx("span", { ...getTokenProps({ token }) }, key))) }, i))) })) }));
    }
    if (withLoc) {
        return (_jsx(Highlight, { code: children.trim(), language: language, theme: theme === "dark"
                ? config.addons.source.themeDark
                : config.addons.source.themeLight, children: ({ className, style, tokens, getLineProps, getTokenProps }) => (_jsx("pre", { className: className, style: {
                    ...style,
                    textAlign: "left",
                    margin: "0.5em 0 1em 0",
                    padding: "1em 0",
